...
{
    fprintf(stderr, "lsi: address_space_read(%08x, flags=%08x, buf=%p, len=%08x)\n", address, flags, buf, len);
    ABORT();
}
...
{
    fprintf(stderr, "lsi: address_space_write(%08x, flags=%08x, buf=%p, len=%08x)\n", address, flags, buf, len);
    ABORT();
}
